  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      {downloads.map((material)=>{
        return <View style={{margin: 20, marginBottom:0}} key={material?._id}>
        <TouchableOpacity style={styles.button} onPress={()=>{
          navigation.navigate('pdf_screen', {uri: material?.pdfUrl});
        }} >
          <View style={{flexDirection: 'column',}}>
            <Text style={styles.subjectText}>{material?.title}</Text>
            <Text style={styles.descText}> {material?.type} - Class : {material?.class}</Text>
          </View>
          <View style={{flexDirection: 'row', alignItems: 'center', paddingHorizontal:10}}>
            <TouchableOpacity onPress={()=>{handleDelete(material)}} >
